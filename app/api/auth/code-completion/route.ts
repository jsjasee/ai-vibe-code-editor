...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileContent, cursorLine, cursorColumn, suggestionType, fileName } =
      body; // destructuring the output from body
...
async function generateSuggestion(prompt: string): Promise<string> {
  try {
    ...
    // 11434 is where our ollama is running, if you go to http://localhost:11434/, you should see 'ollama is running' message.
    const response = await fetch("http://localhost:11434/api/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: "codellama:7b", // name of codellama model, and make sure this model is installed on computer
        prompt,
        stream: false,
        options: {
    ...
    // Clean up the suggestion (IMPORTANT! if not the code will be like ```some code```, we want to remove the backticks!)

...
interface FileExplorerState {
  ...
  handleAddFile: (
    ...
    parentPath: string,
    writeFileSync: (filePath: string, content: string) => Promise<void>, // this is coming from useWebContainer
    instance: any, // also from useWebContainer
    saveTemplateData: (data: TemplateFolder) => Promise<void>, // usePlayground hook
  ) => Promise<void>;
  handleAddFolder: (
...
export const useFileExplorer = create<FileExplorerState>((set, get) => ({
  templateData: null,
  playgroundId: "",
  openFiles: [] satisfies OpenFile[], // this is the state that will store all of our currently opened files in our system
  activeFileId: null, // when we click on a file it will become active
  editorContent: "",
  ...
  openFile: (file) => {
    const fileId = generateFileId(file, get().templateData!); // get() means get the values of the current state.
    const { openFiles } = get();
  ...
  closeFile: (fileId) => {
    const { openFiles, activeFileId } = get(); // use get() if you want to access the store in Zustand aka the store of variables / states.
  ...
  // show indicator like a red dot when the file content is not matching previous content (aka changes were made to the file)
